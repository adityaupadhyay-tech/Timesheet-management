...
// Custom Magnetic Cursor Hook - for element movement (not used in this implementation)
function useMagneticPull(ref: React.RefObject<HTMLElement>, strength: number = 0.1) {
...
function MagneticCursor() {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    magneticElements.forEach((element) => {
      if (isInMagneticZone(element as HTMLElement) && !magneticTarget) {
        handleMagneticEnter(element as HTMLElement)
        foundMagnetic = true
      }
    })
  ...
  const handleMagneticEnter = (target: HTMLElement, strength: number = 0.3) => {
  ...
  const isInMagneticZone = (element: HTMLElement) => {
  ...
  const getElementDimensions = () => {
    if (!magneticTarget) return { width: 24, height: 24 }
    ...
    <>
      ...
      >
        ...
        <div 
          ...
          style={{
            width: magneticTarget ? elementDimensions.width : 24,
            height: magneticTarget ? elementDimensions.height : 24,
...
function MagneticButton({ 
  ...
  onMagneticEnter: (target: HTMLElement, strength: number) => void
