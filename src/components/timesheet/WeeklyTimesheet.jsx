                  ...
                  <div key={entry.id} className="ml-5 p-4 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-3"><div><div><span>
                            {getProjectName(entry.projectId)}
                          </span><span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            entry.status === 'approved' ? 'bg-green-100 text-green-800' === 'rejected' ? 'bg-red-100 text-red-800' === 'submitted' ? 'bg-yellow-100 text-yellow-800' :
